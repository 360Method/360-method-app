  ...
  try {
    ...
    if (subscription.stripe_subscription_id) {
      try {
        ...
        if (needsUpdate) {
          await helper.asServiceRole.entities.UserSubscription.update(subscription.id, updates);
    ...
    if (subscription.stripe_customer_id) {
      const paymentMethods = await helper.asServiceRole.entities.PaymentMethod.filter({
        user_id: user.id,
        is_default: true,
        status: 'active'
      });
      ...
      if (paymentMethods && paymentMethods.length > 0) {
        const pm = paymentMethods[0] as any;
        paymentMethod = {
          brand: pm.card_brand,
          last4: pm.card_last4,
    ...
    return Response.json({
      ...
      tier: subscription.tier || user.user_metadata?.tier || 'free',
      subscription: {
        id: subscription.id,
